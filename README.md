...
|------------|----------------|-----------|-------------------------------------------------------------------------------|
| Free       | $0             | Unlimited | Core review, 1 local AI bot, email notifications, IDE diagnostics, configurable dashboard, REST API |
| Standard   | $10/user/mo    | No cap — billed per user | Cloud AI providers, Compare (file-to-file & full-repo diff), Teams (cross-repo rollups for leads), Slack/Teams/Email/Webhook/SMS notifications, native Task Trackers (YouTrack/Jira/Mantis/Bugzilla), task tracker merge gate, reviewer auto-assignment by load, bus factor guard, Backup, 2FA, review watchers, Migration Safety Analyzer, Secret Scanning, Semantic Duplicate Detection, Dead Symbol Detection |
...
- **Bus factor guard (Standard+)** — additive on top of the above: when a review's changed files have only ever been touched by a handful of people (a configurable owner-count threshold, default 2), force-assign one additional reviewer who has never touched those files, so knowledge spreads before a low-bus-factor area becomes a single point of failure. Never replaces or narrows the normal reviewer selection — off by default, and a no-op until there's enough ownership history to judge.
