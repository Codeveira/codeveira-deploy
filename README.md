...
**Not yet built (a future phase):** all eight originally-scoped languages now ship, and the typed Find-Usages/Go-to-Definition upgrade, diagnostics-inlined-into-diff-hunks, and per-language dependency-install caching (above) have since shipped too. Remaining gaps: GitLab/Bitbucket/self-hosted-VCS-hosted Composer packages for PHP (only GitHub-hosted `dist` downloads are allow-listed); a repo with multiple independent `.csproj` files and no `.sln` for C# (only the first found is restored/analyzed); Gradle support for Java and Kotlin; no automatic eviction or size cap on the new dependency caches (clear one manually via `docker volume rm`); non-GitLab archive fetching; yarn/pnpm lockfile support for TypeScript (npm/`package-lock.json` only today).
